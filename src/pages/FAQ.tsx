...
export const FAQ = () => {
  ...
  return (
    <>
      <Helmet>
        <title>{language === 'ar' ? 'الأسئلة الشائعة - مدرسة الأورمان الثانوية بنين' : 'FAQ - Al-Orman Secondary School for Boys'}</title>
        <meta
          name="description"
          content={language === 'ar' ? 'إجابات على الأسئلة الأكثر شيوعاً حول مدرسة الأورمان الثانوية بنين.' : 'Answers to the most common questions about Al-Orman Secondary School for Boys.'}
        />
        <meta property="og:title" content={language === 'ar' ? 'الأسئلة الشائعة - مدرسة الأورمان الثانوية بنين' : 'FAQ - Al-Orman Secondary School for Boys'} />
        <meta property="og:description" content={language === 'ar' ? 'اكتشف إجابات على الأسئلة الشائعة حول المدرسة، القبول، الرسوم، الأنشطة، والمزيد.' : 'Find answers to frequently asked questions about the school, admissions, fees, activities, and more.'} />
                ...
                <div key={testimonial.id} className="bg-white p-8 rounded-xl shadow-lg border border-gray-100 relative hover:shadow-xl transition-all">
                  ...
                  <div className="flex items-center gap-4 mb-4">
                    {testimonial.image ? (
                      ...
                      <div className="w-16 h-16 rounded-full bg-gray-200 flex items-center justify-center text-gray-400">{testimonial.name[language][0]}</div>
                    )}
                    <div>
